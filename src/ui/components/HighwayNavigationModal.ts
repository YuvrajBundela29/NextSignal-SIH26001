...
export class HighwayNavigationModal {
 ...
 ${step.hazardType ? `<div style="background: rgba(239, 68, 68, 0.15); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 4px; padding: 4px 8px; margin-top: 6px; font-size: 10px; color: #fca5a5;"> Active Hazard: <strong>${step.hazardType}</strong> &bull; Status: ${step.status}</div>` : ''}
 </div>
 `).join('')}
 </div>
 </div>
