...
export class DistrictHud {
  ...
  public render(
    ...
    const levelBg =
      risk.level === 'CRITICAL' ? 'rgba(239, 68, 68, 0.15)' :
      risk.level === 'HIGH' ? 'rgba(249, 115, 22, 0.15)' :
      risk.level === 'MODERATE' ? 'rgba(234, 179, 8, 0.15)' : 'rgba(34, 197, 94, 0.15)';
    ...
    const localizedName = this.getDistrictName(district);
      ...
      <div style="display: flex; flex-direction: column; gap: 10px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #f1f5f9; font-size: 11px;">
        ...
        <div style="background: linear-gradient(135deg, #1c1008 0%, #111827 100%); border: 1px solid #d9770640; border-radius: 10px; padding: 10px;">
          ...
          <div style="font-size: 10px; color: #cbd5e1; line-height: 1.6; background: #0b1120; padding: 8px 10px; border-radius: 6px; border: 1px dashed #374151;">
            ...
            <div>Battalion: <span style="color: #38bdf8; font-weight: 600;">${ndrfOrder.commandingBattalion} (${ndrfOrder.battalionBase})</span></div>
            <div>Staging Helipad: <span style="color: #34d399;">${ndrfOrder.stagingLocation} (${ndrfOrder.helipadCoordinates})</span></div>
            <div>Personnel: <span style="color: #fbbf24; font-weight: 700;">${ndrfOrder.personnelCount} NDRF Rescuers</span></div>
          </div>
        </div>
        ...
        <div style="background: #0f172a; border: 1px solid #1e293b; border-radius: 10px; padding: 10px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 6px;">
            ${relevantGauges.map(g => `
              <div style="background: #0b1120; border-radius: 6px; padding: 6px 8px; border-left: 2px solid ${g.glofRisk === 'CRITICAL' || g.glofRisk === 'HIGH' ? '#ef4444' : g.glofRisk === 'ELEVATED' ? '#f97316' : '#22c55e'}; display: flex; justify-content: space-between; align-items: center;">
                <div>
                  <div style="font-weight: 600; color: #f1f5f9; font-size: 10px;">${g.stationName} (${g.riverName})</div>
                  <div style="font-size: 9px; color: #64748b;">Level: ${g.currentLevelM}m | Warning: ${g.warningLevelM}m</div>
                </div>
                <span style="font-size: 8px; font-weight: 800; color: ${g.glofRisk === 'CRITICAL' || g.glofRisk === 'HIGH' ? '#ef4444' : g.glofRisk === 'ELEVATED' ? '#f97316' : '#22c55e'}; background: ${g.glofRisk === 'CRITICAL' ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.15)'}; padding: 1px 5px; border-radius: 4px;">
                  ${g.glofRisk} (${g.trend})
                </span>
              </div>
            `).join('')}
        ...
        <!-- AI Geohazard Advisory Terminal Note -->
        <div style="background: #06101e; border: 1px solid #0284c730; border-radius: 10px; padding: 10px; border-left: 3px solid #0284c7;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
            <span style="font-weight: 800; font-size: 9px; color: #38bdf8; text-transform: uppercase; letter-spacing: 0.5px;">
              GEOHAZARD AI ADVISORY
            </span>
            <span style="font-size: 8px; color: #64748b;">Deterministic Core</span>
          </div>
          <div style="font-size: 10px; color: #e2e8f0; line-height: 1.5;">
            ${aiAdvisory.analysis || aiAdvisory.civilDefenseAction || 'Baseline slope conditions normal.'}
          </div>
        </div>
    ...
    // Bind copy order
    const btnCopy = document.getElementById('btn-copy-ndrf');
    if (btnCopy) {
      btnCopy.addEventListener('click', () => {
        const text = `[NDRF MOBILIZATION ORDER ${ndrfOrder.orderId}]
Target: ${district.name}, ${district.state}
Risk Score: ${risk.compositeScore}/100 (${risk.level})
Battalion: ${ndrfOrder.commandingBattalion} (${ndrfOrder.battalionBase})
Staging: ${ndrfOrder.stagingLocation} (${ndrfOrder.helipadCoordinates})
Personnel: ${ndrfOrder.personnelCount} Rescuers
Issued: ${ndrfOrder.issuedAt}`;
        navigator.clipboard.writeText(text);
        btnCopy.textContent = 'Copied!';
        btnCopy.style.color = '#34d399';
        setTimeout(() => {
          btnCopy.textContent = 'Copy Draft';
          btnCopy.style.color = '#38bdf8';
        }, 2000);
      });
    }
  }
