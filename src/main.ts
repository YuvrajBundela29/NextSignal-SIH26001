...
import { installLocalApiInterceptor } from '@/services/nextsignal-local-api';
...
// Ensure all local dashboard panels receive rich mock intelligence data in Vite dev
...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(async () => {
      ...
      // NextSignal product layer — initialize after World Monitor App is ready.
      // This is additive: does not replace or break any WM functionality.
      try {
        const { initNextSignal } = await import('./app/nextsignal-orchestrator');
        initNextSignal();
      } catch (nsErr) {
        // NextSignal layer failure is non-fatal — WM dashboard remains functional.
        console.warn('[NextSignal] Orchestrator failed to initialize:', nsErr);
      }
    })
    .catch((error: unknown) => {
      console.error(error);
