...
export type GlobeTexture = 'blue-marble' | 'earth-day' | 'topographic';
...
export const GLOBE_TEXTURE_OPTIONS: { value: GlobeTexture; label: string }[] = [
  { value: 'blue-marble', label: 'Google Earth / Satellite (NASA Blue Marble)' },
  { value: 'earth-day', label: 'Daylight Satellite (HD)' },
  { value: 'topographic', label: 'Topographic Bathymetry' },
];
...
export function getGlobeTexture(): GlobeTexture {
  try {
    ...
    if (raw === 'blue-marble' || raw === 'earth-day' || raw === 'topographic') return raw;
  } catch { /* ignore */ }
  return 'blue-marble';
}
